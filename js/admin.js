...
const getLocalAdminEvents = () => {
  try {
    ...
    return Array.isArray(list) ? list.map((item) => mapEventDoc(item.id, item)) : [];
  } catch {
    return [];
  }
};
...
app.addEventListener('submit', async (event) => {
  ...
  const saveLocalEvent = (newEvent) => {
    try {
      ...
      localStorage.setItem('eventflowEvents', JSON.stringify(list));
    } catch { }
  };
